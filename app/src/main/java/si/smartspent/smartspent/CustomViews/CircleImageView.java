...
public class CircleImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        hasBorder = attributes.getBoolean(R.styleable.CircleImageView_civ_border, false);
        hasSelector = attributes.getBoolean(R.styleable.CircleImageView_civ_selector, false);
        shadowEnabled = attributes.getBoolean(R.styleable.CircleImageView_civ_shadow, SHADOW_ENABLED);

        // Set border properties, if enabled
        if(hasBorder) {
            int defaultBorderSize = (int) (2 * context.getResources().getDisplayMetrics().density + 0.5f);
            setBorderWidth(attributes.getDimensionPixelOffset(R.styleable.CircleImageView_civ_borderWidth, defaultBorderSize));
            setBorderColor(attributes.getColor(R.styleable.CircleImageView_civ_borderColor, Color.WHITE));
        }

        // Set selector properties, if enabled
        if(hasSelector) {
            int defaultSelectorSize = (int) (2 * context.getResources().getDisplayMetrics().density + 0.5f);
            setSelectorColor(attributes.getColor(R.styleable.CircleImageView_civ_selectorColor, Color.TRANSPARENT));
            setSelectorStrokeWidth(attributes.getDimensionPixelOffset(R.styleable.CircleImageView_civ_selectorStrokeWidth, defaultSelectorSize));
            setSelectorStrokeColor(attributes.getColor(R.styleable.CircleImageView_civ_selectorStrokeColor, Color.BLUE));
        }

        // Set shadow properties, if enabled
        if(shadowEnabled) {
            shadowRadius = attributes.getFloat(R.styleable.CircleImageView_civ_shadowRadius, SHADOW_RADIUS);
            shadowDx = attributes.getFloat(R.styleable.CircleImageView_civ_shadowDx, SHADOW_DX);
            shadowDy = attributes.getFloat(R.styleable.CircleImageView_civ_shadowDy, SHADOW_DY);
            shadowColor = attributes.getColor(R.styleable.CircleImageView_civ_shadowColor, SHADOW_COLOR);
    ...
    /**
     * Enables a dark shadow for this CircleImageView.
